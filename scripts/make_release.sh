...
# increase every release. project.yml pins it; override with a monotonic build
# number from the commit count. (Requires a full clone — CI uses fetch-depth: 0.)
...
#   SPARKLE_DOWNLOAD_URL_PREFIX e.g. https://github.com/jasonzh0/Clacky/releases/download/v0.2.0/
#
...
if command -v "$GENERATE_APPCAST" >/dev/null 2>&1; then
  if [[ -z "${SPARKLE_PRIVATE_KEY_FILE:-}" ]]; then
    ...
    GENERATE_ARGS=(
      ...
      --link "https://github.com/jasonzh0/Clacky"
    )
